...
> **注**: 旧 Berkeley URL (`www2.eecs.berkeley.edu/.../BSDS500data.tgz`) は 404 で死んでいる（2026-05-17 確認）。

```bash
# GitHub リポジトリから sparse clone で取得（400 枚 PNG、追加ツール不要）
...
BSD68 は KAIR リポジトリに同梱されていない。`cszn/FFDNet` リポジトリから sparse clone で取得する。

```bash
# BSD400 と同じリポジトリの BSD68/original/ から sparse clone で取得
git clone --depth 1 --filter=blob:none --sparse \
...
| 学習率 | 5e-5（フルの半分） |
| 推定学習時間 | RTX 3060 で約 11 時間（1k iter 実測: 6.6分 → 100k = 11h 換算） |
...
| 作業 | 工数 |
|---|---|
| BSD400・BSD68 ダウンロード・配置 | 完了済み（2026-05-17） |
| `train_scunet_gray.py` 実装 | 完了済み（2026-05-17） |
| fine-tuning 実行（B案） | 約 11 時間（放置可）※実測値から換算 |
| 評価・比較 | 1〜2 時間 |
| **合計（実作業）** | **5〜8 時間** |
